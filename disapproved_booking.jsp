...
<% 
	try 
	{
        ...
        if(i>0)
        {	out.println("<script>alert('Disapproved successfully.');</script>");
         ...
         <h2>Pending Bookings</h2>
            <table>
                ...
                <tbody>
                    <% 
try {
    // Assuming GetterSetter has a method to retrieve cid as int
    int cid = GetterSetter.getCid(); // Retrieve cid as int

    Connection con = ConnectDB.dbCon();
    PreparedStatement ps = con.prepareStatement("SELECT * FROM bookings WHERE bstatus='pending' AND cid=?");
    ps.setInt(1, cid); // Set cid as int parameter
...
%>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="approved_booking.jsp?bid=<%= rs.getInt("bid") %>" button class="btn btn-approve">Approve</a>
                            <!-- Disapprove button can have functionality if needed -->
                           <a href="disapproved_booking.jsp?bid=<%= rs.getInt("bid") %>"   button class="btn btn-disapprove">Disapprove</button>
                        </td>
                    </tr>
                    <% 
                            }
